window.addEventListener('DOMContentLoaded', () => {
  ...
  const getAllDefaultAgendaTitles = () => {
    const customDefaults = Array.isArray(state.agenda?.customDefaults) ? state.agenda.customDefaults : [];
    return [...agendaDefaults, ...customDefaults.filter((title) => typeof title === 'string' && title.trim())];
  };

  const buildDefaultAgendaItems = () =>
    getAllDefaultAgendaTitles().map((title) => ({ id: crypto.randomUUID(), title: title.trim(), completed: false, isDefault: true }));
  ...
  const ensureAgendaForToday = () => {
    ...
    if (!Array.isArray(state.agenda.customDefaults)) {
      state.agenda.customDefaults = [];
    } else {
      state.agenda.customDefaults = state.agenda.customDefaults
    ...
    const requiredDefaults = getAllDefaultAgendaTitles();
    const existingAgendaTitles = new Set(state.agenda.items.map((item) => item.title.toLowerCase()));
    requiredDefaults.forEach((title) => {
      const normalizedTitle = title.toLowerCase();
      if (!existingAgendaTitles.has(normalizedTitle)) {
        state.agenda.items.unshift({
        ...
        ${
          ...
          <li>
            ...
            ${
              item.isDefault
                ? ''
                : `<div class="agenda-item-actions"><button type="button" data-agenda-action="make-default" data-id="${item.id}">Make Default</button><button type="button" data-agenda-action="remove" data-id="${item.id}">Remove</button></div>`
            }
          </li>
        `,
            )
  ...
  elements.agendaCard.addEventListener('click', (event) => {
    ...
    if (action === 'make-default' && !agendaItem.isDefault) {
      agendaItem.isDefault = true;
      if (!state.agenda.customDefaults.some((title) => title.toLowerCase() === agendaItem.title.toLowerCase())) {
        state.agenda.customDefaults.push(agendaItem.title);
      }
    }
